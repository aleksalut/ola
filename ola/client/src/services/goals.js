...
export const complete = async (id) => (await api.post(`/goals/${id}/complete`)).data;

export const updateProgress = async (id, progress) =>
  (await api.patch(`/goals/${id}/progress`, { goalId: Number(id), progress })).data;

export const updateStatus = async (id, status) =>
  (await api.patch(`/goals/${id}/status`, { goalId: Number(id), status })).data;
